...
func (r *Registry) GetKeyValue(ctx context.Context, key string) ([]byte, error) {
	kvPair, _, err := r.client.KV().Get(key, &consul.QueryOptions{})
	if err != nil {
		return nil, err
	}
	if kvPair == nil {
		return nil, errors.New("invalid kv")
	}
	if kvPair.Value == nil {
		return nil, errors.New("null value present")
	}
